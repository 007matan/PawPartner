...
class AuthManager{
    ...
    func signIn(email: String, password: String, completion: @escaping (Result<User, Error>) -> Void) {
        Auth.auth().signIn(withEmail: email, password: password) { authResult, error in
                if let user = authResult?.user {
                    let userData = User(id:user.uid, name: user.displayName ?? "", email: user.email ?? "", password: "", dogs: [])
    ...
    func signUp(name: String, email: String, password: String, completion: @escaping (Result<User, Error>) -> Void) {
        Auth.auth().createUser(withEmail: email, password: password) { authResult, error in
                if let user = authResult?.user {
                    let userData = User(id: user.uid, name: name, email: user.email ?? "", password: "", dogs: [])
                    completion(.success(userData))
                } else if let error = error {
                    completion(.failure(error))
                } else {
